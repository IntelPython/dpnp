...
class TestStackedEigenvalues:
    ...
    def check_eig(self, ew_gpu, ew_cpu, ev_gpu=None, ev_cpu=None):
        ...
        testing.assert_allclose(ew_gpu, ew_cpu, rtol=1e-5, atol=1e-4)
        ...
        if ev_gpu is not None or ev_cpu is not None:
            ...
            scale_vec = numpy.divide(ev_cpu[..., 0], ev_gpu.asnumpy()[..., 0])
            ev_cpu *= scale_vec[..., None]

            testing.assert_allclose(ev_gpu, ev_cpu, rtol=1e-5, atol=1e-4)
