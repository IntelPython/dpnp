...
class TestRaw:
    ...
    def configure(self, backend, in_memory, clean_up, jitify):
        ...
        kern = cupy.RawKernel(
            _test_source1, "test_sum", backend=backend, jitify=jitify
        )
        mod2 = cupy.RawModule(
            code=_test_source2, backend=backend, jitify=jitify
        )
        mod3 = cupy.RawModule(
            code=_test_source3,
    ...
    @pytest.mark.skipif(
        cupy.cuda.runtime.is_hip, reason="HIP uses hsaco, not cubin"
    )
    ...
    @pytest.mark.skipif(
        cupy.cuda.runtime.is_hip, reason="HIP uses hsaco, not ptx"
    )
    ...
    @pytest.mark.skipif(
        not cupy.cuda.runtime.is_hip, reason="CUDA uses cubin/ptx, not hsaco"
    )
    ...
    def test_dynamical_parallelism_compile_failure(self, backend, jitify):
        ...
        ker = cupy.RawKernel(
            _test_source4, "test_kernel", backend=backend, jitify=jitify
        )
        N = 10
        inner_chunk = 2
        x = cupy.zeros((N,), dtype=cupy.float32)
    ...
    def test_cuFloatComplex(self, jitify):
        ...
        mod = cupy.RawModule(
            code=_test_cuComplex, translate_cucomplex=True, jitify=jitify
        )
    ...
    def test_const_memory(self, backend, jitify):
        mod = cupy.RawModule(
            code=test_const_mem, backend=backend, jitify=jitify
        )
    ...
    def test_template_failure(self, backend, jitify):
        ...
        mod = cupy.RawModule(code=test_cxx_template, jitify=jitify)
    ...
    def test_raw_pointer(self, backend, jitify):
        mod = cupy.RawModule(code=test_cast, backend=backend, jitify=jitify)
    ...
    @pytest.mark.skipif(
        cupy.cuda.runtime.is_hip, reason="only CUDA raises warning"
    )
...
class TestRawPicklable:
    ...
    def test_raw_picklable(self, configure):
        ...
        s = subprocess.run(
            [sys.executable, "TestRawPicklable.py"] + test_args, cwd=temp_dir
        )
...
@pytest.mark.skipif(
    cupy.cuda.runtime.is_hip, reason="Jitify does not support ROCm/HIP"
)
class TestRawJitify:
    @pytest.fixture(autouse=True)
    def configure(self):
    ...
    def _helper(self, jitify, header, options=()):
        ...
        code += _test_source1
        mod1 = cupy.RawModule(
            code=code, backend="nvrtc", options=options, jitify=jitify
        )
