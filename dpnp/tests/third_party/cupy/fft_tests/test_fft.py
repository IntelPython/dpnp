...
class TestRfft2:
    ...
    def test_irfft2(self, xp, dtype, order, enable_nd):
        ...
        invalid_s = self.s in [None, (1, 4, 10)]
        invalid_axes = self.axes in [None, (-2, -1), (-3, -2, -1)]
        if invalid_s and invalid_axes and is_gpu_device():
            # If input is not Hermitian symmetric, result is undefined
            # However, dpnp (OneMKL) returns the correct results on cpu
            # so skipping for gpu only
