...
@pytest.mark.parametrize(
    "lhs", [[[6, 5, 4], [3, 2, 1]], [1.3, 2.6, 3.9], 5, 0.5]
)
@pytest.mark.parametrize(
...
class TestEdiff1d:
    @pytest.mark.parametrize(
        "data_type", [numpy.float64, numpy.float32, numpy.int64, numpy.int32]
    )
    @pytest.mark.parametrize(
        "array",
        [
...
class TestTrapz:
    ...
    @pytest.mark.parametrize(
        "data_type_y", [numpy.float64, numpy.float32, numpy.int64, numpy.int32]
    )
    @pytest.mark.parametrize(
        "data_type_x", [numpy.float64, numpy.float32, numpy.int64, numpy.int32]
    )
