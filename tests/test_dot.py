...
def test_dot(dtype, array_info):
    size1, size2, shape1, shape2 = array_info
    a = numpy.array(numpy.random.uniform(-5, 5, size1), dtype=dtype).reshape(
        shape1
    )
    b = numpy.array(numpy.random.uniform(-5, 5, size2), dtype=dtype).reshape(
        shape2
    )
    ia = dpnp.array(a)
    ib = dpnp.array(b)
...
def test_dot_complex(dtype, array_info):
    size1, size2, shape1, shape2 = array_info
...
def test_dot_ndarray(dtype, array_info):
    size1, size2, shape1, shape2 = array_info
    a = numpy.array(numpy.random.uniform(-5, 5, size1), dtype=dtype).reshape(
        shape1
    )
...
def test_dot_out(dtype, array_info):
...
def test_dot_input_dtype_matrix(dtype1, dtype2):
...
def test_dot_out_error_scalar(ia):
...
def test_dot_out_error(shape_pair):
